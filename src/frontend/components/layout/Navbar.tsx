...
import { Menu, UserCircle, Columns, Rows, LayoutDashboard, Users, PackageSearch, ShoppingBag, CalendarCheck, Settings2, Dog, Factory, Truck, LogOut, UtensilsCrossed } from "lucide-react";
...
export function Navbar({ onMenuClick, layoutMode = "vertical", toggleLayout }: NavbarProps) {
  ...
  const getLinksForRole = () => {
    ...
    if (role === "customer" || role === "admin") {
      ...
      links.push({ href: "/recipes", label: "Minhas Receitas", icon: UtensilsCrossed });
    }
  ...
  return (
    <header className="sticky top-0 z-30 flex flex-col border-b bg-background shadow-sm">
        ...
        <div className="flex items-center gap-4">
          {toggleLayout && (
            <Button 
              ...
              title={layoutMode === "horizontal" ? "Mudar para menu lateral" : "Mudar para menu superior"}
              className="hidden lg:flex"
            >
              {layoutMode === "horizontal" ? <Columns className="h-4 w-4" /> : <Rows className="h-4 w-4" />}
            </Button>
          )}
      ...
      {layoutMode === "horizontal" && (
        <div className="hidden lg:flex items-center space-x-1 px-4 sm:px-6 h-12 overflow-x-auto border-t bg-muted/20">
          {navLinks.map((link) => {
