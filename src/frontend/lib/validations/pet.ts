 ...
 * StorePetRequest / UpdatePetRequest rules that apply to the UI, with a few
 * fields (sex, neutered, breed, weight, age) made required client-side only
 * — the backend keeps them nullable so the admin pet modal (which has no
 * sex/neutered fields and no per-field error handling) keeps working.
 *
...
export const petFormSchema = z.object({
  ...
  breed: z.string().trim().min(1).max(255),
  weight: z.number().min(0).max(120),
  age: z.number().int().min(0).max(360),
